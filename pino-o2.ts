...
class OpenobserveTransport extends Transform {
  ...
  constructor(options: TransportOptions) {
    console.log(options, "options11111111111");

    super({ objectMode: true });

    const defaultOptions: Partial<TransportOptions> = {
      batchSize: 1,
      timeThreshold: 1 * 1000,
      silentSuccess: false,
      silentError: false,
    };
  ...
  private createApiUrl(): string {
    ...
    console.log(
      `${parsedUrl.protocol}//${parsedUrl.host}${path}/api/${organization}/${streamName}/_multi`
    );
  ...
  _transform(
    ...
    console.log(this.logs, log, "_transform2222222222");
  ...
  private scheduleSendLogs(): void {
    console.log(this.logs, "scheduleSendLogs");
    ...
    console.log(batchSize, "batchSize", timeThreshold, "timeThreshold");
  ...
  private async sendLogs(): Promise<void> {
    console.log(this.logs, "logs buffered, sending...");
    if (this.logs.length === 0 || this.apiCallInProgress) {
      return;
    }
    ...
    console.log(JSON.stringify(bulkLogs), "bulkLogs");
